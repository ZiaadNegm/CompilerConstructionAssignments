...
static void Usage(char *program) {
  ...
  printf("  --max-factor/-f <value>      Max constant factor for strength "
         "reduction.\n");
...
static int ProcessArgs(int argc, char *argv[]) {
  static struct option long_options[] = {
      ...
      {"max-factor", required_argument, 0, 'f'},
      {0, 0, 0, 0}};
  ...
  while (1) {
    c = getopt_long(argc, argv, "hsvo:b:f:", long_options, &option_index);
    ...
    case 's':
      ...
      break;
    case 'f':
      global.max_factor = (int)strtol(optarg, NULL, 10);
      break;
    case 'o':
